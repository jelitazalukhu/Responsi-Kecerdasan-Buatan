...
# COPAS CLASS NET KAMU DI SINI (CONTOH)
class Net(nn.Module):
    def __init__(self):
        super().__init__()
...
# --- BAGIAN 2: LOAD BOBOT MODEL ---
@st.cache_resource
def load_model():
    model = Net()
    # PENTING: map_location ke CPU karena laptop/cloud biasanya gak pake GPU
    model.load_state_dict(torch.load('animal_classifier.pth', map_location=torch.device('cpu')))
    model.eval()
    return model

# --- BAGIAN 3: TAMPILAN MATCHA AESTHETIC ---
st.set_page_config(page_title="Animal Classifier", page_icon="🐾")

# CSS biar agak Matcha-matcha gitu
st.markdown("""
